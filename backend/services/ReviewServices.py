...
def averageRating(hotel_id):
    result1 = db.session.query(func.avg(Review.rating).label("average rating"),func.count(Review.hotel_id).label('total review')).filter(Review.hotel_id==hotel_id).all()
    result = result1[0]['average rating']
    count = result1[0]['total review']
