...
export const HotelSearchList = () => {
  ...
  const areFiltersAvailable = (arr) => {
    const value = filters.filter((filter) =>
      filter === "breakfastAndDinner"
        ? arr.hotelfacalities[0]?.breakfast && arr.hotelfacalities[0]?.dinner
        : arr.hotelfacalities[0]?.[filter]
    ).length;
  ...
  const getBudgetFilteredArray = (arr) => {
    const filteredArray =
      budgetFilters.length > 0
        ? arr.filter((item) => areBudgetFiltersAvailable(item))
        : arr;
  ...
  useEffect(() => {
    ...
    setViewAll(hotelList.length > 3 ? true : false);
  }, [hotelList]);

  useEffect(() => {
    // Filter Handler for Filter Properties and Budget Filters
    getFilteredArray();
    // setViewAll(filteredHotels.length > 3 ? true : false);
    setViewAll(false);
        ...
        <Stack direction={"column"} spacing={2}>
          {status === "rejected" ? (
            ...
            >
              ...
              <Typography
                variant="h5"
                style={{ fontFamily: "inter", marginTop: "1vh" }}
              >
                {" "}
                Please update your search preferences.
              </Typography>
            </Paper>
          ) : status === "loading" ? (
            <div style={{ marginLeft: "12vw" }}>
              ...
              >
                Wait a moment, we are finding the best hotels for you!
              </Typography>
            </div>
          ) : (
            <>
